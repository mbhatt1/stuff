...
The two regimes are \emph{asymmetric}: horn~(1) is a proven
impossibility; horn~(2) is a theorem limitation---the bound loses
information, but the defense is not proven to succeed either. A
gentle defense ($K$ small) provably fails. An aggressive defense
($K$ large) widens the $\varepsilon$-band and escapes the
persistence proof, but at the cost of distorting the input space.
...
Multi-turn interaction compounds the problem: boundary information
accumulates (\texttt{boundary\_points\_accumulate}), the best
...
boundary fixed points. The $\varepsilon$-robust band scales as
$L(K^n + 1)\delta$.
\end{theorem}
...
Estimated directional slope $G \approx 5$ at boundary crossings;
for $\ell \approx 1$ and $K = 1$, $G > \ell(K+1) = 2$, satisfying
the transversality condition of \Cref{thm:persistent}.
