...
\label{thm:cone}
In $\R$, if $f(x) \geq \tau + c(x - z)$ for all
$x \in (z,\, z + \delta_0)$ with $c > \ell(K+1)$, then
...
Estimated from the 2D behavioral surface in~\cite{munshi2026manifold}: directional slope $G \approx 5$ at the steepest boundary crossing, defense-path Lipschitz constant $\ell \approx 1$ (estimated from grid-adjacent score differences along the defense direction), and $K = 1$ (identity-rate defense). These satisfy $G > \ell(K+1) = 2$.
...
The last uses $K \leq K+1$; we keep the uniform constant $K+1$ for consistency with the displacement bound in \Cref{lem:input-bound}.
\end{proof}
...
\label{thm:crossing}
If $f$ crosses $\tau$ on $[a,b]$ with margin $m$ (i.e.,
$f(a') < \tau - m$ and $f(b') > \tau + m$ for some
$a', b' \in [a,b]$) and $\|f - g\|_\infty \leq \varepsilon < m$,
then $g$ also crosses $\tau$ on $[a,b]$.
\end{theorem}
