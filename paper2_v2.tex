...
($D = \mathrm{id}$, so $K = 1$ and $\ell = 0$). The two runs use
small budgets ($\sim$15 and $\sim$60 evaluations respectively) and
fill only $1$--$2\%$ of the grid; they are smoke-tests of the
validator's logic, not a saturation study. The agreement we report
is what the validator measures on those exact grids.
...
$|\mathcal{S}_{\text{act}}|$ is the actual post-defense persistent set;
TP/FP$_{\text{int}}$/FN are the per-cell confusion-matrix counts.}
...
gpt-3.5-turbo-0125    & $0.50$ & $9$  & $11.44$ & $11.44$ & $7$ & $7$ & $7$ & $\mathbf{0}$ & $0$ \\
\bottomrule
\end{tabular}
\end{table}

On both runs, the predicted steep set matches the actual persistent
set cell-for-cell, with zero interior false positives. Independently,
Theorem~\ref{thm:eps-robust}'s $\varepsilon$-robust bound holds for every filled
cell on every run ($14/14$ and $9/9$, respectively), with worst-case
empirical $|f(D(x)) - \tau|$ comfortably below the predicted
$LK\dist(x, z^{*})$. Theorem~\ref{thm:main}'s boundary fixation is
non-vacuously activated on both runs as well: the closest filled
boundary cell sits at $|f(z^{*}) - \tau| \in \{0.20,\, 0.49\}$, the
discretization gap that vanishes under grid refinement.

The well-aligned target additionally exemplifies the harmless-boundary
case: at $\tau = 0.5$, gpt-5-mini's peak AD is exactly $0.5$ and
$U_\tau = \emptyset$, so the trilemma's preconditions fail and no
impossibility is predicted---the validator correctly reports
``not applicable'' (matching the qualitative GPT-5-Mini observation
above). Lowering $\tau$ to $0.3$ activates the theorem on the same
data and produces the row above. The full per-cell records (each
cell's $f(x)$, $D(x)$, $f(D(x))$, distance to $z^{*}$, $\varepsilon$-robust LHS
and RHS, and steep/persistent membership flags) are emitted as JSON
by the validator and reproduced from the live archives in the
artifact.
...
heatmap), \texttt{trilemma experiment} (just run the experiment), and
\texttt{trilemma synth} (generate a synthetic mesa/mosaic/flat surface
for a self-contained smoke test). Cost on the runs above is dominated
by the gpt-4.1 judge committee and was $\sim$\$1 total.
