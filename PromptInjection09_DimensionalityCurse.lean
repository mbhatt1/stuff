...
theorem defense_fraction_strict_decrease
    ...
    r ^ (d + 1) < r ^ d := by
  rw [pow_succ]
  exact mul_lt_of_lt_one_left (pow_pos hr0 d) hr1
...
theorem balls_needed_tendsto_atTop
  ...
  apply Filter.Tendsto.atTop_div_const hα
  exact scaled_defense_fraction_tendsto_zero (le_of_lt hr0) hr1 (le_of_lt hC)
...
theorem exponential_base_gt_one : 1 < 1 / dc.r := by
  rw [one_div]
  exact one_lt_inv dc.hr_pos dc.hr_lt
