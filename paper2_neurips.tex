...
Theorem~\ref{thm:eps-robust}'s $\varepsilon$-robust bound holds for
every filled cell on every run ($14/14$ and $9/9$, respectively),
with worst-case empirical $|f(D(x)) - \tau|$ comfortably below the
predicted $LK\dist(x, z^{*})$. Theorem~\ref{thm:main}'s boundary
fixation is non-vacuously activated on both runs as well: the closest
filled boundary cell sits at
$|f(z^{*}) - \tau| \in \{0.20,\, 0.49\}$, the discretization gap
that vanishes under grid refinement.
...
cell's $f(x)$, $D(x)$, $f(D(x))$, distance to $z^{*}$,
$\varepsilon$-robust LHS and RHS, and steep/persistent membership
flags) are emitted as JSON by the validator and reproduced from the
live archives in the artifact.
