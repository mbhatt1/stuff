...
\begin{figure}[h]
\centering
\includegraphics[width=0.92\textwidth]{figures/eps_robust_saturated.pdf}
\caption{Per-cell validation of Theorem~\ref{thm:eps-robust} on the
saturated live run. Each of the $82$ filled cells is one dot:
$56$ interior unsafe ($f(x) > \tau$, red),
$16$ safe ($f(x) < \tau$, green),
$10$ boundary cells in $\cl{S_\tau}\setminus S_\tau$ (black squares).
The dashed line is the predicted bound
$LK\dist(x,z^*) + |f(z^*)-\tau|$ with empirically-measured
$LK = 23.63$ and discretization slack $0.375$. Every dot lies below
the bound; the worst empirical $|f(D(x)) - \tau|$ is $0.50$ against a
predicted maximum of $\sim$$30$, a $\sim$$59\times$ margin.
The $66$ red and black cells together are $\mathcal{S}_{\text{pred}} = \mathcal{S}_{\text{act}}$
under the identity defense---predicted and actual persistent sets
coincide cell-for-cell. (Log $y$-axis to show both the dots and the
bound line on the same plot.)}
\label{fig:eps-robust-saturated}
\end{figure}

On the saturated run, every one of the $66$ predicted-persistent
cells is also actually persistent under identity, with zero false
positives and zero false negatives---the predicted set
$\mathcal{S}_{\text{pred}}$ and the empirical set
$\mathcal{S}_{\text{act}}$ are \emph{the same set of $66$ cells},
on a real OpenAI model surface, with no model-specific tuning of
the validator. Theorem~\ref{thm:eps-robust}'s $\varepsilon$-robust
bound also holds for all $82$ filled cells under identity, with
worst-case empirical $|f(D(x)) - \tau|$ of $0.50$ vs. predicted
maximum $LK\dist(x, z^{*}) + |f(z^{*}) - \tau| = 29.30$, a
$\sim$$59\times$ margin. Theorem~\ref{thm:main}'s boundary fixation
is non-vacuously activated: $10$ boundary cells exist in
