...
/--
The behavioral space is the unit interval I = [0,1], considered as a subtype
of R with its inherited topology.
-/
...
theorem continuous_nonConst_has_nonTrivial_levelSet
  ...
  -- We have f(a) ≠ f(b). WLOG assume f(a) < f(b); symmetric otherwise.
  -- Let v = (f(a) + f(b)) / 2.  Then min(f(a),f(b)) < v < max(f(a),f(b)).
  -- By IVT (isConnected_univ and the intermediate value property),
  -- there exists c with f(c) = v.  Since v ≠ f(a) and v ≠ f(b), c ≠ a and c ≠ b.
  -- Now we have three distinct points {a, b, c}, with f(a) ≠ f(c) = v and f(b) ≠ f(c) = v.
  -- Apply IVT again between a and c to find another preimage of f(a), giving the result.
  unfold hasNonTrivialLevelSet
  -- Case split on f(a) < f(b) or f(b) < f(a)
  rcases lt_or_gt_of_ne hfab with hlt | hgt
  · -- Case f(a) < f(b)
    -- Pick the midpoint value
    set v := (f a + f b) / 2 with hv_def
    have hv_gt : f a < v := by linarith
    have hv_lt : v < f b := by linarith
    -- I is connected, so by IVT there exists c in I with f(c) = v
    have hconn : IsPreconnected (Set.univ : Set I) := isPreconnected_univ
    have ha_mem : a ∈ (Set.univ : Set I) := Set.mem_univ a
    have hb_mem : b ∈ (Set.univ : Set I) := Set.mem_univ b
    have hfa_le : f a ≤ v := le_of_lt hv_gt
    have hfb_ge : v ≤ f b := le_of_lt hv_lt
    have := intermediate_value_univ₂ hf (le_of_lt hv_gt) (le_of_lt hv_lt)
    obtain ⟨c, hc⟩ := this
    -- c ≠ a because f(c) = v ≠ f(a)
    have hca : c ≠ a := by
      intro heq; rw [heq] at hc; linarith
    exact ⟨c, a, hca, hc⟩
  · -- Case f(b) < f(a): symmetric
    set v := (f a + f b) / 2 with hv_def
    have hv_lt : f b < v := by linarith
    have hv_gt : v < f a := by linarith
    have := intermediate_value_univ₂ hf (le_of_lt hv_lt) (le_of_lt hv_gt)
    obtain ⟨c, hc⟩ := this
    have hca : c ≠ a := by
      intro heq; rw [heq] at hc; linarith
    exact ⟨c, a, hca, hc⟩
...
theorem ivt_levelSet_nonempty
  ...
  exact intermediate_value_univ₂ hf h_le_0 h_le_1
...
theorem opposite_strategies_share_level
  ...
  · -- a ≤ b, so min = a, max = b
    ...
    rw [hmin] at hv_lower
    rw [hmax] at hv_upper
    exact intermediate_value_univ₂ hf (le_of_lt hv_lower) (le_of_lt hv_upper)
  · -- b < a, so min = b, max = a
