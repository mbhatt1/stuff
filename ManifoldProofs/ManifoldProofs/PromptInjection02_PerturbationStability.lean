...
theorem superlevelset_isOpen
  ...
  refine ⟨(f p - τ) / L, div_pos (by linarith) (by exact_mod_cast hL), ?_⟩
  intro p' hp'
...
theorem injectionRobustnessRadius_pos
  ...
  exact div_pos (by linarith) (by exact_mod_cast hL)
...
theorem injectionRobustnessRadius_mono
  ...
  unfold injectionRobustnessRadius
  apply div_le_div_of_nonneg_right _ (by positivity : (0 : ℝ) ≤ L)
  linarith
