...
`gradient f x`.  We collect the key identity:
  ⟪gradient f x, v⟫ = fderiv ℝ f x v
...
theorem gradient_flow_deriv_eq_norm_sq
    ...
    HasDerivAt (f ∘ γ) (‖gradient f (γ t)‖ ^ 2) t := by
  -- f ∘ γ is differentiable by the chain rule
  have hcomp : HasDerivAt (f ∘ γ) (fderiv ℝ f (γ t) (gradient f (γ t))) t := by
    exact HasFDerivAt.comp_hasDerivAt t (hf.hasFDerivAt) hγ
  -- fderiv ℝ f x (gradient f x) = ⟪∇f, ∇f⟫ = ‖∇f‖²
  have hinner : fderiv ℝ f (γ t) (gradient f (γ t)) = ‖gradient f (γ t)‖ ^ 2 := by
...
theorem gradient_flow_stationary_iff_critical
    ...
    (hf : DifferentiableAt ℝ f (γ t))
    (hγ : HasDerivAt γ (gradient f (γ t)) t) :
    ‖gradient f (γ t)‖ ^ 2 = 0 ↔ gradient f (γ t) = 0 :=
...
theorem gradient_flow_strict_increase
    ...
    (hf : DifferentiableAt ℝ f (γ t))
    (hγ : HasDerivAt γ (gradient f (γ t)) t)
    (hne : gradient f (γ t) ≠ 0) :
    0 < ‖gradient f (γ t)‖ ^ 2 := by
  have hpos : 0 < ‖gradient f (γ t)‖ := by
...
structure GradientFlowTrajectory (f : E → ℝ) (x₀ : E) where
  ...
  path_deriv : ∀ t : ℝ, 0 ≤ t →
    HasDerivAt path (gradient f (path t)) t
...
theorem gradient_flow_gradient_vanishes
    ...
    (T : GradientFlowTrajectory f x₀)
    (hbdd : ∀ x, f x ≤ M) :
    Filter.Tendsto (fun t => ‖gradient f (T.path t)‖) Filter.atTop (nhds 0) := by
  sorry
...
theorem discrete_gradient_ascent_improvement
    ...
    (h_lip_grad : ∀ y, ‖gradient f y - gradient f x‖ ≤ L * ‖y - x‖) :
    f (x + η • gradient f x) ≥ f x + (η - η ^ 2 * L / 2) * ‖gradient f x‖ ^ 2 := by
  sorry
...
theorem discrete_gradient_ascent_nonneg_improvement
    ...
    (hη_small : η ≤ 1 / L)
    (h_lip_grad : ∀ y, ‖gradient f y - gradient f x‖ ≤ L * ‖y - x‖)
    (h_step : f (x + η • gradient f x) ≥
              f x + (η - η ^ 2 * L / 2) * ‖gradient f x‖ ^ 2) :
    f (x + η • gradient f x) ≥ f x := by
  have hgrad_sq : 0 ≤ ‖gradient f x‖ ^ 2 := by positivity
  have hηL : η * L ≤ 1 := by
    have := mul_le_mul_of_nonneg_right hη_small (le_of_lt hL_pos)
    rwa [one_div, inv_mul_cancel₀ (ne_of_gt hL_pos)] at this
  have hcoeff : η - η ^ 2 * L / 2 ≥ 0 := by nlinarith [sq_nonneg η, sq_nonneg L]
  linarith [mul_nonneg hcoeff hgrad_sq]
...
theorem first_order_gradient_expansion
    ...
    (fun η : ℝ => f (x + η • gradient f x) - f x - η * ‖gradient f x‖ ^ 2)
    =o[nhds 0] (fun η => η) := by
  sorry
