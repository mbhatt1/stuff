...
def iterT (Φ : PromptInjectionOperator) : ℕ → Φ.X → Φ.X
  | 0 => id
  | n + 1 => Φ.T ∘ iterT Φ n

@[simp]
theorem iterT_zero (p : Φ.X) : iterT Φ 0 p = p := rfl

@[simp]
theorem iterT_succ (n : ℕ) (p : Φ.X) : iterT Φ (n + 1) p = Φ.T (iterT Φ n p) := rfl

/-- Alternate iteration: applying T after n iterations equals n+1 iterations. -/
theorem iterT_succ' (n : ℕ) (p : Φ.X) : Φ.T (iterT Φ n p) = iterT Φ (n + 1) p := rfl
...
-/
theorem ad_iterT_ge (p : Φ.X) (n : ℕ) : Φ.AD p ≤ Φ.AD (iterT Φ n p) := by
  induction n with
  | zero => simp
  | succ n ih =>
    simp only [iterT_succ]
    exact le_trans ih (Φ.ad_mono (iterT Φ n p))
...
theorem adSeq_convergent (p : Φ.X) :
  ...
  exact ⟨_, tendsto_atTop_ciSup (Φ.adSeq_monotone p) (Φ.adSeq_bddAbove p)⟩
...
theorem adSeq_tendsto_adLimit (p : Φ.X) :
  ...
  exact tendsto_atTop_ciSup (Φ.adSeq_monotone p) (Φ.adSeq_bddAbove p)
...
theorem shifted_adSeq_tendsto (p : Φ.X) :
    Tendsto (fun n => Φ.AD (Φ.T (iterT Φ n p))) atTop (nhds (Φ.adLimit p)) := by
  have : (fun n => Φ.AD (Φ.T (iterT Φ n p))) = (fun n => Φ.adSeq p (n + 1)) := by
    ext n; rfl
  rw [this]
  exact (Φ.adSeq_tendsto_adLimit p).comp (tendsto_add_atTop_nat 1)
...
theorem adSeq_linear_lower_bound
  ...
  | succ n ih =>
    ...
    _ = Φ.AD p + (↑(n + 1) : ℝ) * δ := by push_cast; ring
